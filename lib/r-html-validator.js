...
function rHtmlValidator(el = HTMLElement) {
  ...
  function validateAndCorrectTag(html) {
    for (let i = 0; i < html.split("").length; i++) {
      ...
      let condition1 = html.split("")[i] == "<" && isWord(html.split("")[i + 1]);                                 // 여는 태그일 경우
      let condition2 = html.split("")[i] == "<" && html.split("")[i + 1] == "/" && isWord(html.split("")[i + 2]); // 닫는 태그일 경우

      if (condition1 || condition2) { 

        let tag     = html.slice(i).match(tagRegex)[0];
        console.log(tag);
        ...
        if (currentTag.tag.indexOf("</") != 0) {
          ...
          if (openTagStack.peek().tagName == currentTag.tagName) {
            ...
            let sliceStartingPoint = beforeCloseTag == null ? openTagStack.peek().nextIndex : beforeCloseTag.nextIndex; // slice 시작 지점 // <div> <p></p> <p></p> </p> 상황방지
            ...
            if (openTagCount >= closeTagCount) {
              ...
              if (isLineBreakPoint) {
                ...
                i += insertCloseTag.length + currentTag.tag.length - 1;
              ...
              if (isLineBreakPoint) {
                ...
                i += insertOpenTag.length + currentTag.tag.length - 1;
